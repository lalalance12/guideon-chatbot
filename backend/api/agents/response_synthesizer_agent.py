...
Keep your response friendly, concise, and focused on answering the question.
If there isn't enough information to fully answer the query, acknowledge this and suggest what information might help.
...
"""
    ...
    def _build_structured_prompt(self, query: str, context: Dict[str, Any], format_info: Dict[str, Any]) -> str:
        ...
        # Compile knowledge from knowledge base
        knowledge_parts = []
...
## Instructions:
Create a structured educational response with these sections: {sections_text}
...
"""
    ...
    def _build_role_profile_prompt(self, query: str, context: Dict[str, Any], format_info: Dict[str, Any]) -> str:
        ...
        # Extract agent responses and flow context
        agent_responses = context.get("agent_responses", {})
        flow_context = context.get("flow", {})
        
        # Get the role being discussed
        role = flow_context.get("role", "the role")
        
        # Compile knowledge about the role
        knowledge_parts = []
...
"""
    ...
    def _build_learning_pathway_prompt(self, query: str, context: Dict[str, Any], format_info: Dict[str, Any]) -> str:
        ...
        # Compile learning pathway information
...
"""
    ...
    def _build_role_skills_prompt(self, query: str, context: Dict[str, Any], format_info: Dict[str, Any]) -> str:
        ...
        # Get role description from knowledge base
        role_description = "No detailed role description available."
        if "knowledge_base" in agent_responses and agent_responses["knowledge_base"].get("found", False):
            items = agent_responses["knowledge_base"].get("items", [])
            for item in items:
                if item.get("type") == "role" and role.lower() in item.get("title", "").lower():
    ...
    def _build_role_listing_prompt(self, query: str, context: Dict[str, Any], format_info: Dict[str, Any]) -> str:
        ...
        # Get roles information if available
        roles = []
        if "learning_path" in agent_responses:
    ...
    Encourage the user to specify which role they would like to learn more about.

    ## Response:
    """
        return prompt
    
    # Add new prompt builder method
    ...
    def _build_career_map_prompt(self, query: str, context: Dict[str, Any], format_info: Dict[str, Any]) -> str:
        ...
        # Compile knowledge about career maps
        career_map_info = []
        if kb_response.get("found", False):
            items = kb_response.get("items", [])
            for item in items:
    ...
    def _fallback_response(self, query: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Simple template responses based on intent
        intent = context.get("intent")
        
        if intent == QueryIntent.KNOWLEDGE_BASE_QUERY:
            response = "I can provide information about the PSF-AAI framework, including roles, skills, and career paths. Could you please be more specific about what you'd like to know?"
        elif intent == QueryIntent.LEARNING_PATHWAY:
            response = "I can help create a learning pathway for your career goals in analytics and AI. What specific role are you interested in?"
        elif intent == QueryIntent.COURSE_SEARCH:
            response = "I can recommend courses and learning resources for skills in the analytics and AI domains. What specific skills are you looking to develop?"
        else:
            response = "I'm here to help with questions about the PSF-AAI framework, career pathways, and learning resources. How can I assist you today?"
